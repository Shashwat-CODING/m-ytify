...
export const idFromURL = (link: string | null) => {
  ...
  const match = link.match(/(https?:\/\/)?((www\.)?(youtube(-nocookie)?|youtube.googleapis)\.com.*(v\/|v=|vi=|vi\/|e\/|embed\/|user\/.*\/u\/\d+\/)|youtu\.be\/)([_0-9a-zA-Z-]+)/i)?.[7];
  return match?.slice(0, 11) || link.slice(0, 11);
};

export const sanitizeVideoId = (id: string | null | undefined): string => {
  if (!id) return '';
  if (id.length === 11) return id;
  const extracted = idFromURL(id);
  return extracted ? extracted.slice(0, 11) : id.slice(0, 11);
};
