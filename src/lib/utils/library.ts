...
export const syncLibrary = (action: 'add' | 'remove' | 'schedule' | 'init', _id?: string) => {
  import('@modules/muzoSync')
    .then(m => {
      if (action === 'schedule') m.scheduleMuzoSync();
      else if (action === 'init' && m.isCloudSyncEnabled()) m.pullMuzoUserData();
    });
};
...
export async function fetchCollection(
  ...
  document.title = display + ' - m-ytify';
