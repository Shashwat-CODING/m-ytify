    ...
    >
      ...
      >
        ...
        <li tabindex="4" onclick={async () => {
          const targetId = store.actionsMenu?.id || playerStore.stream.id;
          if (!targetId) return;

          try {
            const { fetchPrimaryStream, playPrimaryStream } = await import('@modules/primaryStream');
            const streamData = await fetchPrimaryStream(targetId);
            if (streamData && streamData.lossless) {
              const currentTime = playerStore.audio.currentTime;
              const wasPlaying = playerStore.playbackState === 'playing';
              await playPrimaryStream(streamData, true);
              if (currentTime > 0) {
            ...
            setStore('snackbar', 'Failed to switch to lossless');
          } finally {
            closeDialog();
          }
        }}>
          <span
            ...
            style={{
              ...
              "-webkit-mask-image": "url('https://upload.wikimedia.org/wikipedia/commons/thumb/0/0d/Apple_Lossless_logo.svg/330px-Apple_Lossless_logo.svg.png?utm_source=commons.wikimedia.org&utm_campaign=index&utm_content=thumbnail&_=20230318102657')",
              "mask-image": "url('https://upload.wikimedia.org/wikipedia/commons/thumb/0/0d/Apple_Lossless_logo.svg/330px-Apple_Lossless_logo.svg.png?utm_source=commons.wikimedia.org&utm_campaign=index&utm_content=thumbnail&_=20230318102657')",
